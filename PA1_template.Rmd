...
The mean steps per day for the data with NAs replace (values for original data in parenthesis) is `r round(meansteps.NArep)` (`r round(meansteps)`) and median `r round(mediansteps.NArep)` (`r round(mediansteps`).  

Let's plot a histogram.

```{r stepsogram.NAr,fig.height=4}
hist(stepsperday.NAr$steps,breaks=20,xlab="Steps per day",main="Histogram of steps per day, NAs replaced")
